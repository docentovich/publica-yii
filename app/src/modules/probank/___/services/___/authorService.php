...
class authorService extends Services
{
    ...
    public function save()
    {
        ...
        if ($model->load( Yii::$app->request->specialist() )) {
            if( Yii::$app->user->can("updatePost", ["specialist" => $model]) ) //апдейтим только свои посты
                if($model->save())
                    return true;
        }
    ...
    {
        $post_requst = Yii::$app->request->specialist();
    ...
    public function getByIdPost($id)
    {
        ...
        if(!Yii::$app->user->can("reedPost", ['specialist' => $this->items])){
