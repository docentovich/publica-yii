...
use users\dto\UserTransportModel;
...
class UserService extends BaseUserService
{
    ...
    private function actionRegistration(UserServiceConfig $config)
    {
        ...
        if ($config->userFormModel->load(\Yii::$app->request->post()) && $config->userFormModel->validate()) {
            /** @var UsersUser $user */
            $user = UsersUser::registerNewUser($config->userFormModel->toArray());
     ...
     *
     * @param UsersUser $user
     * @return bool
     * @throws \Exception
     */
    private function saveUser(UsersUser $user)
    {
        if (!$user->validate()) {
            return false;
        }
