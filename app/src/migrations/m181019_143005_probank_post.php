...
class m181019_143005_probank_post extends Migration
{
    ...
    public function safeUp()
    {
        ...
        $this->createIndex('fk_tbl_post_user_id','{{%probank_post}}',['user_id'],false);
        $this->createIndex('image_id','{{%probank_post}}',['image_id'],false);
        $this->createIndex('city_id','{{%probank_post}}',['city_id'],false);

    }

    public function safeDown()
    {
        $this->dropIndex('event_at', '{{%probank_post}}');
        $this->dropIndex('category_id', '{{%probank_post}}');
        $this->dropIndex('fk_tbl_post_user_id', '{{%probank_post}}');
        $this->dropIndex('image_id', '{{%probank_post}}');
        $this->dropIndex('city_id', '{{%probank_post}}');
        $this->dropTable('{{%probank_post}}');
    }
}
